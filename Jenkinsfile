pipeline {
    ...
    stages {
        ...
        stage('Create Kubernetes Secret for IBM Container Registry') {
            steps {
                script {
                    bat 'kubectl get secret ibm-cloud-secret || kubectl create secret docker-registry ibm-cloud-secret --docker-server=icr.io --docker-username=iamapikey --docker-password=%IBM_API_KEY% --docker-email=kiranbcse2025@gmail.com'
                }
            }
        }
